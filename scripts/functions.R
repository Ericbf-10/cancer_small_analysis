...
# Adapted from: https://github.com/Michorlab/tnbc_scrnaseq
...
# Extracted from: https://github.com/Michorlab/tnbc_scrnaseq
# decide whether the cells with markers are epithelial by the rules (i.e. if it has 2 marker genes or more)
...
expression_one_epithelial_marker <- function(mat_here, is_epithelial, epithelial_markers, type_test, thresh_percent){
  ...
  for (i in 1:length(is_epithelial)) {
    ...
    if (is_epithelial[i] == 0) {
      ...
      if (!is.null(epithelial_markers[[i]])) {
        ...
        patient <- sub("_.*$", "", name_cell)
        all_cells_pat <- colnames(mat_here)[grepl(paste0("^", patient, "_"), colnames(mat_here))]
        ...
        for (j in 1:length(epithelial_markers[[i]])) {
          ...
          exprs_j_pats <- mat_here[rownames(mat_here) == epithelial_markers[[i]][j], which(is_epithelial == 1), drop = FALSE]
          exprs_j_pats <- as.numeric(exprs_j_pats[, idx_cells_pat_here, drop = TRUE])
          w_pats <- ecdf(exprs_j_pats)
          cdfs_pats[j] <- w_pats(exprs_j_pats[j])
          
          exprs_j_all <- mat_here[rownames(mat_here) == epithelial_markers[[i]][j], which(is_epithelial == 1), drop = FALSE]
          exprs_j_all <- as.numeric(exprs_j_all)
